...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Entradas Ativas', href: '/entries/active' },
];
...
async function loadActiveEntries() {
    ...
    try {
        ...
        const response = await entryApi.getActiveEntries(params);
        entries.value = Array.isArray(response) ? response : response.data || [];
    } catch (err) {
        console.error('Erro ao carregar entradas ativas:', err);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Entradas Ativas</h1>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        Entradas que ainda não foram concluídas ou canceladas ({{ entries.length }} entradas)
                    </p>
                </div>
                ...
                <div class="flex gap-2">
                    <Dialog v-model:open="isFilterDialogOpen">
                        ...
                        <DialogContent class="sm:max-w-md">
                            <DialogHeader>
                                <DialogTitle>Filtrar Entradas Ativas</DialogTitle>
                            </DialogHeader>
                    ...
                    <Button @click="loadActiveEntries" variant="outline" :disabled="loading">
                        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    <div v-else-if="entries.length === 0" class="py-12 text-center text-muted-foreground">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-foreground">Nenhuma entrada ativa encontrada</h3>
                        <p class="mt-1 text-sm text-muted-foreground">Todas as entradas foram concluídas ou canceladas, ou ajuste seus filtros.</p>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-border">
                            <thead class="bg-muted/50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-muted-foreground uppercase">Criado Por</th>
                                ...
                                <tr v-for="entry in entries" :key="entry.id" class="hover:bg-muted/30">
                                    ...
                                    <td class="px-6 py-4 text-sm whitespace-nowrap text-card-foreground">
                                        <div class="flex flex-col gap-1">
                                            ...
                                            <div v-if="entry.scheduled_exam_date" class="text-xs text-muted-foreground">
