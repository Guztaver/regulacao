...
interface CancelledEntry extends Entry {
    cancelled: boolean;
}
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Entradas Canceladas', href: '/entries/cancelled' },
];

const entries = ref<CancelledEntry[]>([]);
const loading = ref(false);
const message = ref('');
const error = ref('');
const isFilterDialogOpen = ref(false);
const isEntryInfoModalOpen = ref(false);
const selectedEntry = ref<CancelledEntry | null>(null);
...
function loadCancelledEntries() {
    ...
    entryApi
        .getCancelledEntries(params)
        .then((response) => {
            entries.value = response as CancelledEntry[];
        })
        .catch((err) => {
            console.error(err);
...
function showEntryInfo(entry: CancelledEntry): void {
    selectedEntry.value = entry;
    isEntryInfoModalOpen.value = true;
}

function getCreatorName(entry: CancelledEntry): string {
...
<template>
    <Head title="Entradas Canceladas" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Entradas Canceladas</h1>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Visualize e gerencie entradas de pacientes canceladas</p>
                </div>
                ...
                <div class="flex gap-2">
                    <Dialog v-model:open="isFilterDialogOpen">
                        ...
                        <DialogContent class="sm:max-w-md">
                            <DialogHeader>
                                <DialogTitle>Filtrar Entradas Canceladas</DialogTitle>
                            </DialogHeader>
                            ...
                            <div class="grid gap-4 py-4">
                                ...
                                <div>
                                    ...
                                    <Input type="text" v-model="tempFilters.entry_id" placeholder="Digite o ID específico da entrada" class="mt-1" />
                                </div>

                                <div>
                                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Limitar resultados</label>
                    ...
                    <Button @click="loadCancelledEntries" variant="outline" :disabled="loading">
                        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Entradas Canceladas ({{ entries.length }})</CardTitle>
                </CardHeader>
                <CardContent>
